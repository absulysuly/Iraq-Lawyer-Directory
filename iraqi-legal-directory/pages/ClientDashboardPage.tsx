...
const ConversationItem: React.FC<{ conversation: Conversation, onSelect: () => void }> = ({ conversation, onSelect }) => {
    ...
    const isUnread = user?.type === 'client' && !conversation.isReadByClient;
        ...
        >
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">{t.conversationWith}: {conversation.lawyer.name}</p>
                </div>
...
const ClientDashboardPage: React.FC = () => {
    ...
    const handleSelectConversation = (conversation: Conversation) => {
        ...
        if (!conversation.isReadByClient) {
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 max-w-4xl mx-auto">
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{t.clientDashboard}</h1>
            <p className="text-xl text-gray-600 dark:text-gray-400 mb-8">{t.welcome}, {user?.name}!</p>

            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md">
                 <h2 className="text-2xl font-bold p-6 border-b dark:border-gray-700">{t.myConversations}</h2>
                {renderContent()}
            </div>
             <ConversationModal
